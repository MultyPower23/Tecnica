...
public class ordenar {
  ...
  public static void seleccion(byte[] v) {
    for (int i = 0; i < v.length - 1; i++) {
      int menor = i; // asumimos que este es el menor

      for (int j = i + 1; j < v.length; j++) {
        if (v[j] < v[menor]) {
          menor = j; // encontramos uno mas chico, actualizamos
        }
      }

      // Intercambiamos el menor encontrado con la posicion actual
      byte temp = v[i];
      v[i] = v[menor];
      v[menor] = temp;
    }
    imprimir("Seleccion", v);
  }

  // Metodo auxiliar solo para imprimir bonito
  public static void imprimir(String nombre, byte[] v) {
