...
- `alumnos.sql` → tablas `alumnos` y `examenes` relacionadas por llave foránea, con `INNER JOIN` entre ambas.
- `producto.sql` → tablas `producto` y `marca` relacionadas por llave foránea, con `INSERT`, `UPDATE`, `DELETE` y `INNER JOIN`.

## ⚠️ Errores comunes / cosas a las que prestar atención

- `DELETE` y `UPDATE` sin `WHERE` afectan a toda la tabla, hay que tener cuidado.
